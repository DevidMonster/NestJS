...
import { useFetchAllPostQuery, useDeletePostMutation } from '../../../services/post.service';
...
const PostList = () => {
    ...
    const { data, isLoading, refetch } = useFetchAllPostQuery(`?_q=${searchDebounce || ''}`)
    ...
    const columns = [
        ...
        },
        {
            ...
            key: 'title',
            render: (_, { title, _id }) => (
                <Link to={'/admin/posts/edit/' + _id}>{title}</Link>
            )
        },
        {
            title: 'SubTitle',
        ...
        },
        {
            ...
            render: (_, record) => (
                <Space size="middle">
                    <Link to={'/admin/posts/edit/' + record.id}>Edit</Link>
                    <Popconfirm
                        title="Delete post"
    ...
    return <div>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <h1>Posts</h1>
            <Input.Search style={{ width: '300px' }} placeholder='Search here...' onChange={e => setSearchValue(e.target.value)}/>
        </div>
